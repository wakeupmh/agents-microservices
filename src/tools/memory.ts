...
export const memoryTool = createTool({
	...
	parameters: z.object({
		action: z
			.enum(["store", "retrieve"])
			.describe("Whether to store or retrieve a record"),
		patient_id: z.string().describe("The patient identifier"),
		record_id: z
			.string()
			.optional()
			.describe(
				"The record identifier (required for store, optional for retrieve)",
			),
		data: z
			.any()
			.optional()
			.describe("The medical data to store (required for store action)"),
	}),
