...
export const eventsTool = createTool({
	name: "createEvent",
	description:
		"Create medical events in EventBridge for appointments, alerts, and reviews",
	parameters: z.object({
		event_type: z
			.enum(["appointment", "alert", "review"])
			.describe("Type of medical event to create"),
		...
		urgency: z
			...
			.describe("Urgency level of the event"),
		reasoning: z
			.string()
			.describe("Justification for the decision and recommendation"),
	}),
	execute: async ({
		event_type,
		patient_id,
