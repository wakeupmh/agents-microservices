...
export const handler = async (event: AppointmentEvent) => {
	try {
		...
		const { patient_id, event_type, specialist, urgency, reasoning } =
			medicalEvent;
...
async function handleAlert(event: MedicalEvent) {
	...
	console.log(
		`URGENT: Patient ${event.patient_id} requires immediate attention`,
	);
